...
################################################################################
    ...
    def focus( self ):
		if self.window_number == -1:
			self.window_number = lookup_window_number(self.window);
		vim.command(str(self.window_number) + 'wincmd w')
...
################################################################################
    ...
    def execute_cmd( self, _cmd = None, _null_terminate = 1, _passive = 0 ):

        dbg_print ( 'execute_cmd: Enter cmd is ' + str( _cmd ) )
...
################################################################################
    ...
    def exit_shell( self ):
        ...
        try:
            ...
            dbg_print( 'exit_shell: exception' )
...
################################################################################
    ...
    def handle_exit_cmd( self, _cmd ):
        ...
        try:
            ...
            ## At some point in the try block the shell decided to exit, prob
            ## async via SIGCHLD.

            dbg_print( 'handle_exit_cmd: exception thrown' )
...
def lookup_window_number( _window ):
	for i in range(len(vim.windows)):
		if _window == vim.windows[i]:
			return i
	return -1
